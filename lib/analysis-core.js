const clamp = (value, min, max) => Math.min(Math.max(value, min), max);

const compact = (values) => values.filter(Boolean);

export const normalizeArtist = (artist) => ({
  id: artist.id,
  name: artist.name,
  type: artist.type || "Artysta",
  country: artist.country || artist.area?.name || null,
  disambiguation: artist.disambiguation || null,
  score: Number(artist.score || 0),
  tags: (artist.tags || [])
    .sort((left, right) => (right.count || 0) - (left.count || 0))
    .slice(0, 4)
    .map((tag) => tag.name)
});

export const normalizeCity = (city) => ({
  id: city.id,
  name: city.name,
  country: city.country || null,
  countryCode: city.country_code?.toUpperCase() || null,
  admin: city.admin1 || null,
  latitude: city.latitude,
  longitude: city.longitude,
  population: Number(city.population || 0),
  timezone: city.timezone || null
});

export const normalizeArtistHistory = (events = [], artistId, resultCount = 0) => {
  const normalizedEvents = events
    .filter((event) => event.relations?.some((relation) => relation.artist?.id === artistId))
    .map((event) => {
      const place = event.relations?.find((relation) => relation.place)?.place;
      return {
        id: event.id,
        name: event.name,
        type: event.type || "Wydarzenie",
        date: event["life-span"]?.begin || null,
        venue: place?.name || null
      };
    })
    .sort((left, right) => String(right.date || "").localeCompare(String(left.date || "")));

  return {
    events: normalizedEvents,
    documentedCount: normalizedEvents.length,
    resultCount: Number(resultCount || normalizedEvents.length)
  };
};

export const buildAnalysis = ({
  artist,
  city,
  artistHistory = { events: [], documentedCount: 0 },
  events = [],
  liveData = false,
  spotify = { configured: false, artist: null }
}) => {
  const populationFactor = city.population
    ? clamp(Math.log10(city.population) / 7, 0.45, 1)
    : 0.58;
  const identityFactor = clamp(artist.score / 100, 0.45, 1);
  const metadataFactor = clamp((artist.tags.length + 2) / 6, 0.4, 1);
  const artistEvents = events.filter((event) =>
    event.attractions?.some((name) => name.toLocaleLowerCase() === artist.name.toLocaleLowerCase())
  );
  const competitorEvents = events.filter((event) => !artistEvents.includes(event));
  const historyFactor = clamp(Math.log10((artistHistory.documentedCount || 0) + 1) / 2, 0, 1);
  const eventFactor = liveData
    ? clamp(artistEvents.length / 3, 0, 1)
    : 0.2 + historyFactor * 0.35;

  const hasSpotify = spotify && spotify.artist && typeof spotify.artist.popularity === 'number' && spotify.artist.popularity !== null;

  const spotifyFactor = hasSpotify
    ? clamp(Number(spotify.artist.popularity) / 100, 0.3, 1)
    : 0.5;

  let score;
  if (hasSpotify) {
    score = Math.round(clamp(
      38 + identityFactor * 15 + populationFactor * 13 + metadataFactor * 8 + eventFactor * 7 + spotifyFactor * 12,
      45,
      93
    ));
  } else {
    score = Math.round(clamp(
      42 + identityFactor * 18 + populationFactor * 16 + metadataFactor * 9 + eventFactor * 8,
      45,
      93
    ));
  }

  let attendanceMultiplier = 1;
  if (hasSpotify && typeof spotify.artist.followers === 'number' && spotify.artist.followers > 1000) {
    attendanceMultiplier = 1 + Math.log10(spotify.artist.followers / 1000) * 0.15;
  }

  const baseAttendance = (280 + populationFactor * 720 + identityFactor * 430 + metadataFactor * 240) * attendanceMultiplier;
  const p50 = Math.round(baseAttendance / 10) * 10;
  const p10 = Math.round((p50 * 0.62) / 10) * 10;
  const p90 = Math.round((p50 * 1.38) / 10) * 10;
  const venueMin = Math.max(200, Math.round((p50 * 0.85) / 100) * 100);
  const venueMax = Math.max(venueMin + 200, Math.round((p90 * 0.95) / 100) * 100);

  let verdict = "Warto sprawdzić ofertę";
  let verdictCopy = "Artysta i rynek zostały zweryfikowane. Prognoza wymaga jeszcze danych sprzedażowych promotera.";

  if (score >= 78) {
    verdict = "Mocny kandydat do shortlisty";
    verdictCopy = "Dopasowanie artysty i wielkość rynku uzasadniają przejście do kalkulacji kosztów oraz terminu.";
  } else if (score < 65) {
    verdict = "Potrzebna ostrożna kalkulacja";
    verdictCopy = "Sygnały bazowe są ograniczone. Zacznij od mniejszego venue i konserwatywnego kosztorysu.";
  }

  const locationLabel = compact([city.admin, city.country]).join(", ");
  const tagsLabel = artist.tags.length ? artist.tags.join(", ") : "brak tagów gatunkowych";
  const latestEvent = artistHistory.events?.[0] || null;
  const latestEventLabel = latestEvent
    ? compact([latestEvent.date, latestEvent.name, latestEvent.venue]).join(" · ")
    : "Brak wydarzeń z potwierdzoną relacją wykonawcy.";

  return {
    score,
    verdict,
    verdictCopy,
    recommendation: `Rozważ venue na ${venueMin}–${venueMax} osób`,
    attendance: { p10, p50, p90 },
    artist,
    city,
    market: {
      population: city.population || null,
      locationLabel,
      competitionCount: liveData ? competitorEvents.length : null,
      artistEventCount: liveData ? artistEvents.length : null,
      documentedArtistEvents: artistHistory.documentedCount || 0,
      latestArtistEvent: latestEvent
    },
    spotify,
    coverage: liveData ? "live" : "partial",
    signals: [
      {
        title: `Zweryfikowany artysta: ${artist.name}`,
        copy: compact([
          compact([artist.type, artist.country, artist.disambiguation]).join(" · "),
          `Profil: ${tagsLabel}`
        ]).join(". ")
      },
      {
        title: `Historia live: ${artistHistory.documentedCount || 0} wydarzeń`,
        copy: latestEventLabel
      },
      {
        title: `Rynek: ${city.name}`,
        copy: city.population
          ? `${locationLabel}. Populacja miasta: ${city.population.toLocaleString("pl-PL")}.`
          : `${locationLabel}. Lokalizacja została zweryfikowana geograficznie.`
      }
    ],
    sources: compact([
      { name: "MusicBrainz", kind: "artist-and-events" },
      { name: "Open-Meteo Geocoding", kind: "city" },
      spotify.artist ? { name: "Spotify", kind: "catalog-reference" } : null,
      liveData ? { name: "Ticketmaster Discovery", kind: "events" } : null
    ])
  };
};
